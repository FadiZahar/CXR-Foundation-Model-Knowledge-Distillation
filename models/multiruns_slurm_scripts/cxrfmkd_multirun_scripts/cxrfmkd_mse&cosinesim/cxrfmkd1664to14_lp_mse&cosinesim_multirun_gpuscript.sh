...
#SBATCH --gres=gpu:1             
#SBATCH --output=/vol/biomedic3/bglocker/mscproj24/fz221/cxr-fmkd/models/multiruns_slurm_scripts/multiruns_slurm_logs/cxrfmkd/cxrfmkd_mse&cosinesim/cxrfmkd1664to14_lp_mse&cosinesim/%x.%N.%j.log  
#SBATCH --time=0-05:00:00 
#SBATCH --nodelist=loki 

# Accepts multirun_seed from the command line; --job-name replaces the %x in --output above
#SBATCH --job-name=cxrfmkd1664to14_lp_mse&cosinesim_multirun
...
python /vol/biomedic3/bglocker/mscproj24/fz221/cxr-fmkd/models/disease_prediction__CXR_FMKD_1664to14__linear_probing.py --kd_type 'MSEandCosineSim' --multirun_seed "$1" --config "$2"

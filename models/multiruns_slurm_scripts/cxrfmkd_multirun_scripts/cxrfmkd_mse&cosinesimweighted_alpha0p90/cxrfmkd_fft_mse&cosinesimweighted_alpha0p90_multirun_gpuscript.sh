...
#SBATCH --gres=gpu:1             
#SBATCH --output=/vol/biomedic3/bglocker/mscproj24/fz221/cxr-fmkd/models/multiruns_slurm_scripts/multiruns_slurm_logs/cxrfmkd/cxrfmkd_mse&cosinesimweighted_alpha0p90/cxrfmkd_fft_mse&cosinesimweighted_alpha0p90/%x.%N.%j.log  
#SBATCH --time=0-05:00:00 
# #SBATCH --nodelist=luna 

# Accepts multirun_seed from the command line; --job-name replaces the %x in --output above
#SBATCH --job-name=cxrfmkd_fft_mse&cosinesimweighted_alpha0p90_multirun
...
python /vol/biomedic3/bglocker/mscproj24/fz221/cxr-fmkd/models/disease_prediction__CXR_FMKD__full_finetuning.py --kd_type 'MSEandCosineSimWeighted-alpha0p90' --multirun_seed "$1" --config "$2"

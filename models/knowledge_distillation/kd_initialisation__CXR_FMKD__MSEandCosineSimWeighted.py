...
class Pre_CXR_FMKD(LightningModule):
    def __init__(self, learning_rate: float, embedding_size: int, alpha: float):
        super().__init__()
...
def main(hparams):
    ...
    model = model_type(learning_rate=LEARNING_RATE, embedding_size=CXRFM_EMBEDS_SIZE, alpha=hparams.alpha)

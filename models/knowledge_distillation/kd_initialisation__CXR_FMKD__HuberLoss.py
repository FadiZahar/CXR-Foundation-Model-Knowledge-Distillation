...
DEV_SPLIT = [0.7, 0.3]
EPOCHS = 40
pre_OUT_DIR_NAME = 'CXR-FMKD_KD-initialisation-HuberLoss/'
...
class Pre_CXR_FMKD(LightningModule):
    ...
    def process_batch(self, batch):
        ...
        # Calculate Mean Absolute Error (MAE) Loss between output embeddings from DenseNet-169 and target embeddings from CXR-FM
        loss = F.huber_loss(output_embeds, target_embeds, delta=1.0)
        return loss
...
def main(hparams):
    ...
    trainer = Trainer(
        ...
        callbacks=[ModelCheckpoint(monitor='val_loss', 
                                   mode='min', 
                                   filename='best-checkpoint_pre-CXR-FMKD_HuberLoss_{epoch}-{val_loss:.4f}',

...
DEV_SPLIT = [0.7, 0.3]
EPOCHS = 40
pre_OUT_DIR_NAME = 'CXR-FMKD_KD-initialisation-MSEandCosineSimLearned/'
...
class Pre_CXR_FMKD(LightningModule):
    def __init__(self, learning_rate: float, embedding_size: int):
        ...
        # Learnable parameters for adjusting loss function weights dynamically based on variance
        self.log_var_mse = nn.Parameter(torch.zeros(1), requires_grad=True)
        self.log_var_cosinesim = nn.Parameter(torch.zeros(1), requires_grad=True)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Log the variance parameters
        self.log('log_var_mse', self.log_var_mse.item(), on_step=True, on_epoch=True, logger=True)
        self.log('log_var_cosinesim', self.log_var_cosinesim.item(), on_step=True, on_epoch=True, logger=True)
...
def main(hparams):
    ...
    KD_TYPE_DIR_NAME = "KD-MSEandCosineSimLearned"
    ...
    trainer = Trainer(
        ...
        callbacks=[ModelCheckpoint(monitor='val_loss', 
                                   mode='min', 
                                   filename='best-checkpoint_pre-CXR-FMKD_MSEandCosineSimLearned_{epoch}-{val_loss:.4f}',

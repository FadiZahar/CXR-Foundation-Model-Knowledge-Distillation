...
#SBATCH --gres=gpu:1             
#SBATCH --output=/vol/biomedic3/bglocker/mscproj24/fz221/cxr-fmkd/models/knowledge_distillation/multiruns_slurm_scripts/multiruns_slurm_logs/cxrfmkd_kdinit_mse&cosinesim/%x.%N.%j.log  
#SBATCH --time=1-00:00:00 
#SBATCH --nodelist=luna        

# Accepts multirun_seed from the command line; --job-name replaces the %x in --output above
#SBATCH --job-name=cxrfmkd_kdinit_mse&cosinesim_multirun
...
python /vol/biomedic3/bglocker/mscproj24/fz221/cxr-fmkd/models/knowledge_distillation/kd_initialisation__CXR_FMKD__MSEandCosineSim.py --multirun_seed "$1" --config "$2"

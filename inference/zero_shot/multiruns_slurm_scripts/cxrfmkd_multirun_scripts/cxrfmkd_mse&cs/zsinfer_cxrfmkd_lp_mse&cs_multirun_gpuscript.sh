...
#SBATCH --gres=gpu:1             
#SBATCH --output=/vol/biomedic3/bglocker/mscproj24/fz221/cxr-fmkd/inference/zero_shot/multiruns_slurm_scripts/multiruns_slurm_logs/zsinfer_cxrfmkd/cxrfmkd_mse&cs/cxrfmkd_lp_mse&cs/%x.%N.%j.log  
#SBATCH --time=0-05:00:00 
# #SBATCH --nodelist=loki  

# Accepts multirun_seed from the command line; --job-name replaces the %x in --output above
#SBATCH --job-name=zsinfer_cxrfmkd_lp_mse&cs_multirun
...
python /vol/biomedic3/bglocker/mscproj24/fz221/cxr-fmkd/inference/zero_shot/zero_shot_inference__CXR_FMKD__linear_probing.py --kd_type 'MSEandCS' --multirun_seed "$1" --inference_on "$2"

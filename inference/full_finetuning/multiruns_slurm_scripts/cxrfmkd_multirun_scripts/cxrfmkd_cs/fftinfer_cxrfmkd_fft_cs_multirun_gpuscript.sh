...
#SBATCH --gres=gpu:1             
#SBATCH --output=/vol/biomedic3/bglocker/mscproj24/fz221/cxr-fmkd/inference/full_finetuning/multiruns_slurm_scripts/multiruns_slurm_logs/fftinfer_cxrfmkd/cxrfmkd_cs/cxrfmkd_fft_cs/%x.%N.%j.log  
#SBATCH --time=0-05:00:00
# #SBATCH --nodelist=luna 

# Accepts multirun_seed from the command line; --job-name replaces the %x in --output above
#SBATCH --job-name=fftinfer_cxrfmkd_fft_cs_multirun
...
python /vol/biomedic3/bglocker/mscproj24/fz221/cxr-fmkd/inference/full_finetuning/full_finetuning_inference__CXR_FMKD__full_finetuning.py --kd_type 'CS' --multirun_seed "$1" --inference_on "$2"

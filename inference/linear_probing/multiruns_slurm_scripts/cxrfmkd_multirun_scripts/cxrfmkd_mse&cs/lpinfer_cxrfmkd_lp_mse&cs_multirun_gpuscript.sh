...
#SBATCH --gres=gpu:1             
#SBATCH --output=/vol/biomedic3/bglocker/mscproj24/fz221/cxr-fmkd/inference/linear_probing/multiruns_slurm_scripts/multiruns_slurm_logs/lpinfer_cxrfmkd/cxrfmkd_mse&cs/cxrfmkd_lp_mse&cs/%x.%N.%j.log  
#SBATCH --time=0-05:00:00
# #SBATCH --nodelist=luna 

# Accepts multirun_seed from the command line; --job-name replaces the %x in --output above
#SBATCH --job-name=lpinfer_cxrfmkd_lp_mse&cs_multirun
...
python /vol/biomedic3/bglocker/mscproj24/fz221/cxr-fmkd/inference/linear_probing/linear_probing_inference__CXR_FMKD__linear_probing.py --kd_type 'MSEandCS' --multirun_seed "$1" --inference_on "$2"
